...
- An `APPROVED` review must exist on the *exact current head commit*: the paired agent for an agent-authored PR, or both agents for an automation PR. The wrapper also requires the review's immutable `submitted_at` to be at or after any force-push that installed the current head. GitHub can rewrite historical review `commit_id` and `updated_at` fields after a force-push, so matching `commit_id` alone is not fresh-review evidence.
...
The author-specific noreply paths for both agent accounts have been exercised end to end. The wrapper still verifies the resulting commit email after every merge rather than treating account configuration as permanent evidence.
...
**Branch protection is enabled.** `main` requires four CI checks (`Desired-state catalog`, `Terraform and Kubernetes manifests`, `Container runtime contracts`, `Portal build and contracts`), CODEOWNERS approval, stale-review dismissal on new pushes, and linear history; force-pushes and deletions are disabled. The rules in this document are therefore structurally enforced rather than a matter of discipline. Stale-review dismissal is why every handoff cites an *exact head*: amending a PR discards the prior approval by design, and the wrapper independently re-checks that the paired approval matches the current head and its force-push freshness boundary.

**GitHub mutates historical review metadata after force-push.** During the PR #56 rebase sequence, historical approvals kept their original `submitted_at` but GitHub rewrote their `commit_id` and `updated_at` to the new head. Branch-protection stale-review dismissal and an exact `commit_id` check therefore did not prove freshness. The merge wrapper now compares each required approval's immutable submission time with the issue timeline's `head_ref_force_pushed` event for the current head. Issue #63 records the incident and regression contract.
