...
**(c) Merge commit author.** GitHub's squash-merge API can derive the new commit's author from account profile state rather than the reviewed source commit. This caused the 2026-08-02 merge-attribution incident and, on 2026-08-02, GitHub also rejected the canonical `j2d3` noreply address when passed through the squash-author API. The `hack/merge-pr.sh` wrapper therefore requires exactly one source commit, verifies both its author and committer against the PR author's canonical noreply identity, and uses an exact-head rebase merge. Post-merge verification confirms the resulting author identity.

**Structural note on isolation.** Separate git worktrees do *not* isolate global `gh` auth state on macOS, and per-agent `GH_CONFIG_DIR` does not isolate Keychain-backed tokens. The full multi-agent isolation on one machine requires: (i) per-agent isolated clones, (ii) per-agent `GH_CONFIG_DIR` with `gh auth login --insecure-storage`, (iii) per-clone SSH host aliases (`github.com-<user>`) so `git push` uses the intended key regardless of `gh` state, and (iv) a verified single-commit rebase merge so account profile metadata is never consulted for commit authorship. Convenience wrappers on any given laptop that combine these are helpful shortcuts, not normative — a new contributor on a different machine must be able to satisfy the portable rules above with plain `gh` and `git`.
...
- Every request must contain exactly one commit whose author and committer match the PR author's canonical noreply identity. It is merged with `--rebase --delete-branch --match-head-commit`, preserving reviewed source attribution without profile-email selection.
