...
4. Obtain review and merge. All PRs are merged via `./hack/merge-pr.sh <pr-number>`, which requires exactly one verified noreply-authored commit and enforces exact-head review, green CI, and a linear rebase merge. Agent-authored PRs require the paired agent; `github-actions[bot]` lifecycle PRs require both agents. Meta-tooling PRs additionally require an issue #6 notice before opening (per [COLLABORATION.md](COLLABORATION.md)). Do not apply Kubernetes application manifests from a workstation.
