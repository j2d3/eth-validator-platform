...
| **Networks** | Hoodi plus a pinned Ephemery generation; both stopped by default |
...
| Project home and operator portal | Responsive, public-safe visual shell and typed specialist-surface registry under `control-plane/portal`; read-only by contract | Public project home at `https://g.j2d3.com` with no identity gate. The curated aggregate-telemetry API (`/api/status`, from PR #77) is served on the operations subdomain at `https://ops.g.j2d3.com/api/status`. Grafana is on the same operations subdomain at `https://ops.g.j2d3.com/grafana` behind Grafana's own admin login | OIDC-backed Grafana operator auth (deferred; tracker to be filed) |
...
| Real Geth/Lighthouse pair | Catalog-generated Hoodi and generation-pinned Ephemery Flux HelmReleases plus a manual GitHub Actions activate/stop PR form; active means EL + beacon node only and signing remains disabled. Ephemery verifies its immutable release bundle, initializes generation-specific chain data, and cannot render validator duties | **Offline/runtime-contract evidence only.** Stopped/active chart contracts, catalog-projection drift tests, exact bundle SHA verification, and throwaway non-root Geth custom-genesis initialization pass; no Flux lifecycle transition or client sync has been observed | Exercise one stopped → active → stopped cycle through Flux, starting with Ephemery for a short sync rehearsal, and capture runtime evidence (Phase 3) |
...
Nothing in the repository authorizes validator signing by default. The local profile is `platform-smoke`; its Hoodi and Ephemery node assignments are stopped, the shared signer remains Hoodi-only, and artifact-mode validator duties are rejected.
...
| [Network profiles](docs/runbooks/network-profiles.md) | How immutable Hoodi/Ephemery identity, custom artifacts, resets, and signer boundaries are represented. |
...
| `control-plane/portal` | Public project home at `https://g.j2d3.com`; curated aggregate-telemetry API separately served on the operations subdomain at `https://ops.g.j2d3.com/api/status` |
