...
Public visibility is a deliberate portfolio and disclosure choice, not the shape a production operator repository would take. A production instance would be private and — more importantly — the cluster would not authenticate to Git at all: CI would render, validate, and cosign-sign the manifests, publish them as an OCI artifact to ECR, and Flux would reconcile from ECR through EKS Pod Identity. That moves the cluster-edge credential from a static Git secret to a short-lived AWS role, removes the "cluster holds a long-lived Git credential" problem entirely, and lets Flux gate reconciliation on the signer identity that produced the artifact. The full comparison of the two shapes — visibility, cluster-edge credential, provenance gating, bootstrap, rotation, blast radius, and merge policy — is captured in [production-evolution.md — Repository visibility and the Git-source credential](docs/production-evolution.md).

The other half of the choice is what a repository has to look like to *be* safe to share: **no credentials, no secret values, no personal identifiers**. This lab intentionally publishes its infrastructure *shape* (Terraform, manifests, chart values) so a reader can inspect the design, and retains the legacy `platform.galaxy-lab` API/label namespace tracked for migration in issue #68. That audit standard — credentials, secret values, and personal identifiers absent — applies whether a repository is public or private; the public artifact is the evidence.
...
```mermaid
flowchart TD
    repo["GitHub repository<br/>desired state"] --> flux["Flux<br/>reconciliation"]
