...
- Authors merge their own PRs through `./hack/merge-pr.sh`, which fails closed unless the paired agent approved the exact current head, CI is green, and the branch contains exactly one commit with the author's verified noreply identity. The wrapper uses an exact-head rebase merge so GitHub never chooses author metadata from profile state. A lifecycle PR authored by `github-actions[bot]` is the narrow ownership exception: both agents approve it, then either may run the same verified path.
