...
#
#   1. The authenticated GitHub login must equal the PR author
#      (authors merge their own PRs).
#   2. Only j2d3 and 5u6r054 are recognized authors; unknown authors
#      fail closed. The wrapper enforces the specific pairing:
#      j2d3-authored PRs require an approval from 5u6r054, and
#      5u6r054-authored PRs require an approval from j2d3.
#   3. The PR must be OPEN, non-draft, and mergeable.
#   4. The paired agent's LATEST review must be APPROVED on the exact
#      current head commit. Evaluating only the latest review from that
...
#   6. Merge is squash with --match-head-commit and mandatory
...
#      a noreply address, requiring a history rewrite.
...
# Recognized PR author -> paired reviewer required by the collaboration contract.
paired_reviewer_for() {
  case "$1" in
    j2d3)    printf '5u6r054' ;;
    5u6r054) printf 'j2d3' ;;
...
pr_json="$(gh pr view "$PR_NUMBER" --repo "$REPO" --json \
  author,headRefName,headRefOid,state,isDraft,mergeable,mergeStateStatus,reviewDecision)" \
...
# 4. Known-author check + author noreply email + paired reviewer.
expected_email="$(author_email_for "$pr_author")" \
  || fail "unknown PR author '$pr_author'; wrapper recognizes only j2d3 and 5u6r054"
required_reviewer="$(paired_reviewer_for "$pr_author")" \
  || fail "unknown PR author '$pr_author'; cannot determine paired reviewer"
...
# 6. Paired reviewer's LATEST review state must be APPROVED on the exact
#    current head, AND the PR's aggregate reviewDecision must be APPROVED.
#    Evaluating only the latest review from that reviewer prevents a later
#    CHANGES_REQUESTED from silently being ignored just because a historical
#    APPROVED record still exists in the reviews list.
paired_latest="$(gh api "/repos/$REPO/pulls/$PR_NUMBER/reviews" \
  | jq --arg reviewer "$required_reviewer" \
    '[.[] | select(.user.login == $reviewer)] | if length == 0 then null else last end')" \
  || fail "gh api pulls/$PR_NUMBER/reviews failed"

[[ "$paired_latest" != 'null' ]] \
  || fail "paired agent '$required_reviewer' has not reviewed PR #$PR_NUMBER"

paired_state="$(jq -r '.state' <<<"$paired_latest")"
paired_commit="$(jq -r '.commit_id' <<<"$paired_latest")"

[[ "$paired_state" == 'APPROVED' ]] \
  || fail "paired agent '$required_reviewer' latest review state is $paired_state (need APPROVED); a later CHANGES_REQUESTED supersedes an earlier APPROVED even on the same head"

[[ "$paired_commit" == "$head_oid" ]] \
  || fail "paired agent '$required_reviewer' latest APPROVED is on commit ${paired_commit:0:7}, not current head ${head_oid:0:7}; re-request review on the amended head"
...
# 8. Merge with all mandatory flags.
printf 'merging PR #%s\n  author:   %s\n  head:     %s\n  reviewer: %s\n  email:    %s\n' \
  "$PR_NUMBER" "$pr_author" "$head_oid" "$required_reviewer" "$expected_email"
