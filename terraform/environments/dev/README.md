...
| Public Ethereum networking | Exact AWS Load Balancer Controller chart/image, Pod Identity role, and one selected mixed TCP/UDP P2P Service; all HTTP, metrics, Engine API, and signer ports remain internal | Existing pairs formed outbound peer sets. The legacy controller rejection is observed; the replacement controller/NLB remains unqualified until this reviewed Terraform and Flux slice is applied | One NLB hostname, five listeners, healthy targets, external TCP/UDP traffic, advertised-address behavior, stop deletion, and resume recreation |
