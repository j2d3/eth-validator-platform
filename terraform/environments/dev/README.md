...
| Public Ethereum networking | Exact AWS Load Balancer Controller chart/image, Pod Identity role, and one selected mixed TCP/UDP P2P Service; all HTTP, metrics, Engine API, and signer ports remain internal | The controller and NLB were created, but the first reconciliation proved that same-number TCP/UDP ports require the controller's explicit combined-listener annotation; public P2P remains unqualified until that correction is observed | One NLB hostname, two `TCP_UDP` listeners plus one UDP listener, healthy targets, external TCP/UDP traffic, advertised-address behavior, stop deletion, and resume recreation |
...
- one identity-addressed container per validator signing key. The first five are
  `.../signing/validator-keystore` and
  `.../signing/validator-keystore-02`, and
  `.../signing/validator-keystore-03`, and
  `.../signing/validator-keystore-04`, and
  `.../signing/validator-keystore-05`. Each stores one encrypted testnet
