...
| Public Ethereum networking | Exact AWS Load Balancer Controller chart/image, Pod Identity role, and one selected mixed TCP/UDP P2P Service; all HTTP, metrics, Engine API, and signer ports remain internal | One internet-facing NLB exposes `TCP_UDP` listeners on 30303 and 9000 plus UDP on 9001. All three Pod-IP target groups were healthy, external TCP reached 30303 and 9000, and VPC Flow Logs recorded accepted bounded UDP probes to the Pod IP on those two combined listeners. | Valid Ethereum-protocol inbound peer attribution, 9001 UDP traffic, advertised-address behavior, stop deletion, and resume recreation |
